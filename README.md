...
- [x] 文件系统工具：支持读取、写入、补丁修改、目录浏览和文本搜索
- [x] 联网网页搜索与抓取：通过 Tavily API 搜索网页并抓取正文
- [x] 工作区 Shell 执行：支持在受控工作区内执行终端命令，每个会话使用隔离 workspace，且每次从该 workspace 根目录开始
- [x] 技能系统：支持通过 `SKILL.md` 定义技能，并按需加载技能说明
- [x] 技能管理：支持在 Web UI 中统一管理本地技能目录与远程 ClawHub 技能源，支持搜索、安装、更新、移除和刷新技能
- [x] MCP 服务管理：支持在 Web UI 中维护 MCP Server 配置，覆盖新增、编辑、删除、工具同步和列表启停开关
- [x] MCP 工具动态装载：主代理与子代理会自动加载所有启用中的 MCP 工具，并在对话轮次开始时注入运行时工具集
...
- [ ] 更丰富的 MCP 集成：支持资源、Prompt、鉴权策略模板和更多上下文源接入
...
- **技能驱动扩展**：通过极简 `SKILL.md` 把新能力以声明式方式接入主系统；主 Agent 只在命中场景时按需加载技能正文。
- **MCP 动态能力层**：启用中的 MCP Server 会在每轮对话前自动解析为运行时工具，并同时提供给主代理和 `sub_task` 子代理使用。
...
├── api/                  # FastAPI 路由（会话、技能、MCP）
├── core/                 # LangGraph、LLM、技能管理、子代理工厂
├── service/              # 工作流、会话与 MCP 服务管理
├── store/                # SQLite 持久化（含会话与 MCP 配置）
├── tools/                # 文件、搜索、终端等本地工具
└── utils/                # SSE 与日志等辅助组件
skills/                   # 已安装技能目录
```
